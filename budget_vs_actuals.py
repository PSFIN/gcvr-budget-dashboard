...
Output:
  ...
  - budget_vs_actuals_YYYY-MM-DD.html  (visual report saved to this folder)
"""
...
def extract_monthly_revenue(result):
    """Returns list of 12 monthly revenue values.
...
print(f"\n{'='*60}")
print(f"  GCVR 2026 Budget vs Actuals  —  Generated {TODAY_STR}")
print(f"{'='*60}\n")
...
for prop in KEYDATA_PROPERTIES:
    ...
    try:
        ...
        monthly = extract_monthly_revenue(result)
...
html_path = os.path.join(OUTPUT_FOLDER, f"budget_vs_actuals_{REPORT_DATE}.html")
...
# ── Patch dashboard.html with fresh actuals ────────────────────────────────────
...
if os.path.exists(DASHBOARD_PATH):
    ...
    for prop_name, js_name in PROP_JS_NAMES.items():
        ...
        if n:
            print(f"  dashboard.html ← {prop_name} patched")

    # Update GENERATED date and CURRENT_MONTH
